...
CYAN = '\033[96m'
MAGENTA = '\033[95m'
ID = input(f"{YELLOW}{BOLD}𝐈𝐃 :  ")
token = input(f"{RED}{BOLD}𝐓𝐨𝐤𝐞𝐧: ")
from requests import post as pp
from user_agent import generate_user_agent as gg
from random import choice as cc
from random import randrange as rr
import re
yy='asdfbgqwertyuioplkjhmnbvcxz'
ids=[]
def tll():
  try:
...
def check(email):
  ...
  try:
    ...
    tt=(f"\r{GREEN}〱𝗢𝗞:{GREEN} {hits} {RED} :{RED} {bads_instgram} {YELLOW} Email 〱𝗖𝗣 :{YELLOW} {bads_email}")
    print(tt)
    if pp == '0':
      headers = {
        'accept': '*/*',
...
def qqq():
  while True:
    try:
      ...
      username = requests.post('https://www.instagram.com/api/graphql', headers=headers, data=data).json()['data']['user']['username']#['@kesr']
